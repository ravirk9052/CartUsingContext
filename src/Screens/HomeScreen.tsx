...
const ProductItem = ({product}: {product: IObject}) => {
  const data = useContext(cartContext);
  const {onAddCartData, onIncrement, onDecrement, count, addedCartdata} = data;
  const {
    title,
    quantity,
    total,
  ...
  const onPressCartButton = (item: IObject) => {
    setAddCart(true);
    item.itemCount = 1;
    onAddCartData(item);
  };

  const onPressPlusBtn = (item: IObject, getCount: number) => {
    setCount(getCount + 1);
    item.itemCount = getCount + 1;
    onIncrement(getCount);
  };

  const onPressMinusBtn = (item: IObject, getCount: number) => {
    // console.log("73",item)
    setCount(getCount - 1);
    if (getCount > 1) {
      item.itemCount = getCount - 1;
      onDecrement(getCount);
    } else {
      setAddCart(false);
      setCount(1);
      item.itemCount = getCount - 1;
  ...
  return (
    <View style={styles.container}>
      <View style={styles.imgStyle}>
        {/* <Icons name="cross" style={styles.crossed} /> */}
        ...
        {!addCart &&  (
          <View style={styles.AddCartBtn}>
            <TouchableOpacity
              ...
              onPress={() => onPressCartButton(product)}>
        ...
        {addCart && (
          <View style={styles.plusMinusContainer}>
            <TouchableOpacity style={styles.plusContainer}>
              <Icons
                name="minus"
                onPress={() => onPressMinusBtn(product, getCount)}
                size={20}
              />
            </TouchableOpacity>
            ...
            <TouchableOpacity style={styles.plusContainer}>
              <Icons
                name="plus"
                onPress={() => onPressPlusBtn(product, getCount)}
                size={20}
              />
            </TouchableOpacity>
          </View>
...
const HomeScreen = () => {
  const [getStateCartData, setStateCartData] = useState([]);
  const [changedText, setChangedText] = useState('');

  const getCartData = async () => {
    try {
      const jsonData = await fetch('https://dummyjson.com/carts');
      const data = await jsonData.json();
      const newData: any = [];
      for (let eachCart of data.carts) {
        for (let eachItem of eachCart.products) {
          newData.push(eachItem);
        }
      }
      setStateCartData(newData);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const filterProducts = getStateCartData.filter(eachProduct => {
    return eachProduct.title.toLowerCase().includes(changedText.toLowerCase());
  });
    ...
    <SafeAreaView>
      <View style={styles.searchContainer}>
        ...
        <TextInput
          ...
          value={changedText}
          style={styles.input}
          onChangeText={setChangedText}></TextInput>
      </View>
